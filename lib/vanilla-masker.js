(function(root, factory) {
  ...
  var DIGIT = "9",
      ...
      buildOutputWithOptionals = function(pattern, values) {
        var completePatternCharsCount = pattern.replace(/[\W]/g, '').length;
        var optionalCount = pattern.replace(/[^?]/g, '').length;
        if (values.length > completePatternCharsCount)
          return pattern.replace(/\?/, '').split("");
        if (values.length > (completePatternCharsCount - optionalCount)) {
            var diff = completePatternCharsCount - values.length + 1;
            for(var i = 0; i < diff; i++) {
              pattern = pattern.replace(/\?/, '');
            }
        }
        var outputWithoutOptionals = pattern.replace(/[\w][\?]/g, '').split("");
        return outputWithoutOptionals;
    }
  ;
  ...
  VMasker.toPattern = function(value, opts) {
    var pattern = (typeof opts === 'object' ? opts.pattern : opts),
        ...
        output = buildOutputWithOptionals(pattern, values),
